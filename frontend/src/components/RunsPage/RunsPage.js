...
const RunsPage = (props) => {
    ...
    runs.length > 0 ? (
      <div>
        {runs.map((run) => (
          <div
            id="runindexitem"
            onClick={() => viewRun(run.startPosition, run.endPosition)}
          >
            ...
            <div id="eachrun">
              ...
              Pace:{" "}
              {run.distance > 0
                ? formatTime(run.duration / run.distance)
                : "00:00"}{" "}
              time/mile
            </div>
  ...
  const toggleRunStart = !runStarted ? (
    <div id="characterrunspage-startrun">
      <button type="submit" className="runstartstoptoggle" onClick={startRun}>
        Start Run
      </button>
    </div>
  ) : (
    <div id="characterrunspage-endrun">
      <button type="submit" className="runstartstoptoggle" onClick={endRun}>
        End Run
      </button>
    </div>
  );
        ...
        <div id="runsdata-container">
          ...
          <div id="characterrunspage-currentrunandindex">
            <div id="characterrunspage-currentruncontainer">
              <div id="characterrunspage-togglebutton">{toggleRunStart}</div>
              <div id="currentrun-data">
                <div>
                  Start Position:{" "}
                  {startLng
                    ? `[${Number(startLat.toFixed(4))}, ${Number(
                        startLng.toFixed(4)
                      )}]`
                    : ""}
                </div>
                <div>
                  End Position:{" "}
                  {endLng
                    ? `[${Number(endLat.toFixed(4))}, ${Number(
                        endLng.toFixed(4)
                      )}]`
                    : ""}
                </div>
                <div>
                  Time: {endTime ? formatTime(endTime - startTime) : ""}
                </div>
                <div>
                  Distance:{" "}
                  {endLng
            ...
            <div id="characterrunspage-runindexcontainer">
              ...
              <div id="runsindex">{runsIndex}</div>
            </div>
          </div>
        </div>
      </div>
